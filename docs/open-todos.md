...
- [ ] Definir el dominio principal del proyecto.
  Impacta en `Route 53`, `ACM`, subdominios `app.<dominio>` y `api.<dominio>`, consulta formal a Mercado Pago para Saldo LUK + payouts a sellers y configuración real de Turnstile.
...
- [ ] Preparar la consulta formal a Mercado Pago con datos reales.
  Falta reemplazar el placeholder `https://[tu-dominio]` por dominio real y adjuntar capturas del flujo de carga de saldo, conexión OAuth de sellers, payouts a billeteras Mercado Pago, URLs reales del sitio, payload técnico relevante y datos societarios/fiscales aplicables.
...
5. Preparar consulta formal a Mercado Pago para Saldo LUK + payouts con URLs reales.
